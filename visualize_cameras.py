...
def visualize_cameras(colored_camera_dicts, sphere_radius, dataset, camera_size=0.1, geometry_file=None, geometry_type='mesh'):
    #sphere = o3d.geometry.TriangleMesh.create_sphere(radius=sphere_radius, resolution=10)
    #phere = o3d.geometry.LineSet.create_from_triangle_mesh(sphere)
    ...
    for color, camera_dict in colored_camera_dicts:
        ...
        cnt = 0
        frustums = []
        for img_name in sorted(camera_dict):

            print(img_name)
            K = dataset.camera.K
            img_size = dataset.camera.width, dataset.camera.height
